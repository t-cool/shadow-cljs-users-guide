...
////
JS の依存関係の欠落に関連するエラーに遭遇するかもしれません。ほとんどの ClojureScript ライブラリは、使用する `npm` パッケージをまだ宣言していません。なぜなら、彼らはまだ <<cljsjs, CLJSJS>> を使用することを期待しているからです。私たちは `npm` を直接使用したいと考えています。つまり、ライブラリが適切に `:npm-deps` を宣言するまで、 `npm` パッケージを手動でインストールする必要があります。
...
////
独自の nREPL サーバを提供する他のツール（例：`lein`）に `shadow-cljs` を組み込んで使用する場合は、 `shadow-cljs` ミドルウェアを設定する必要があります。そうしないと、CLJ とCLJS の REPL の間で切り替えることができません。
...
////
JS の世界は今でも急速に進化しており、誰もが同じ方法でコードを書いたり配布したりしているわけではありません。
