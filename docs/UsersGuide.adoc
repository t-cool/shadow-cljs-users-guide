...
:author: Thomas Heller , Tony Kay (邦訳: Hiroki Noguchi)
:revdate: Jan 10, 2018
:revnumber: 1.0
:lang: en
:encoding: UTF-8
...
////
JS の世界は今でも急速に進化しており、誰もが同じ方法でコードを書いたり配布したりしているわけではないので、`shadow-cljs` が自動的に回避できないものもあります。これらは通常、カスタムの `:resolve` 設定で解決できますが、バグや見落としもあるかもしれません。
